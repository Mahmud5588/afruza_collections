...
class AdminCategoryBloc extends Bloc<AdminCategoryEvent, AdminCategoryState> {
  AdminCategoryBloc(
      this._getCategories, this._createCategory, this._deleteCategory, this._updateCategory)
  ...
  Future<void> _onLoadCategories(
    ...
    try {
      ...
      emit(state.copyWith(status: AdminCategoryStatus.failure, message: message));
    }
  }
  ...
  Future<void> _onCreateCategory(
    ...
    try {
      await _createCategory(name: event.name);
  ...
  Future<void> _onUpdateCategory(
    ...
    emit(state.copyWith(status: AdminCategoryStatus.loading, message: null));
    try {
      await _updateCategory(categoryId: event.categoryId, name: event.name);
        final result = await _getCategories(limit: _pageSize);
      emit(state.copyWith(
          status: AdminCategoryStatus.success, categories: result.items));
    } catch (error) {
